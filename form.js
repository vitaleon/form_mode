...
CodeMirror.defineMode("form", function(config) {
  ...
  function tokenBase(stream, state) {
    ...
    if (ch == "[") {
      return chain(readQuoted("]", "string", ch == "["), stream, state);
    } else if (ch == '"') {
      return chain(readQuoted(ch, "string", ch == '"'), stream, state);
    } else if (ch == '`') {
      return chain(readQuoted("'", "string-2", ch == '`'), stream, state);
    } else if (ch == "'") {
      return chain(readQuoted(ch, "string-2", ch == "'"), stream, state);
    } else if (ch == ".") {
      stream.eatWhile(/\w/);
  ...
  return {
    startState: function(basecolumn) {
      return {tokenize: [tokenBase],
              ...
              context: {type: "top", indented: (basecolumn || 0)-config.indentUnit},
              continuedLine: false,
              lastTok: null,
              varList: false};
    },
...
CodeMirror.defineMIME("text/x-form", {name: "FORM", mime: "text/x-form", mode: "form", ext: ["frm"]});
